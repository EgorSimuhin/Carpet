...
class DataProcessing():
    ...
    def sumExpWithTwoGauss(self, x, a, b, c, d, *p): #Возможная функция2
        return  self.twogauss(x, *p) + self.exp(x, a, b, c, d)
    
    def sumExpWithGauss(self, x, a, b, c, d, A, x0, s):
        return self.gauss(x, A, x0, s) + self.exp(x, a, b, c, d)

    def PlotTwoGauss(self, x, p):
        n = len(p)//3
        for i in range(n):
            plt.plot(x, self.gauss(x,p[i*3],p[i*3+1],p[i*3+2]), 'b', linewidth=1, label=f'{i}-гаусс')

    def PlotTwoGaussAndExp(self, x, p):
    ...
    def PlotGaussAndExp(self, x, a, b, c, d, A, x0, s):
        plt.plot(x, self.exp(x, a, b, c, d), color='red', linewidth=1, label='Экспонента')
        plt.plot(x, self.gauss(x, A, x0, s), color='orange', linewidth=1, label='Первый гаусс')
    ...
    def ChargePictureSumExpWithTwoGauss(self):
        ch = self.getterdata()[0]
        X = np.arange(0, len(self.getterdata()[0]), 1)
        maxElement = ch.max()
        maxIndex = np.argmax(ch) 
        ...
        X = np.arange(0, len(ch), 1)
        p_ch, cov_ch = curve_fit(self.sumExpWithTwoGauss, X, ch, p0=ip0, bounds=([0]*len(ip0), top_limits))
        slope, ic, r_value, p_value, std_err = stats.linregress(ch, self.sumExpWithTwoGauss(X, *p_ch))
        return p_ch, r_value
    ...
    def ChargePictureSumExpWithGauss(self):
        ...
        maxIndex = np.argmax(ch)
        maxSecondElement, maxSecondIndex = sm.findSecondMax(self.q, ch, maxElement)
        ip0 = [maxSecondElement, 0, maxSecondIndex, 0, maxElement, maxIndex, 10]
        top_limits = [self.limits] * 7
        X = np.arange(0, len(ch), 1)
    ...
    def AmplePictureSumExpWithTwoGauss(self):
        ap = self.getterdata()[1]
        X = np.arange(0, len(self.getterdata()[1]), 1)
        maxElement = ap.max()
        maxIndex = np.argmax(ap) 
        ...
        top_limits = [self.limits] * 10
        X = np.arange(0, len(ap), 1)
        p_ap, cov_ap = curve_fit(self.sumExpWithTwoGauss, X, ap, p0=ip0, bounds=([0]*len(ip0), top_limits))
        slope, ic, r_value, p_value, std_err = stats.linregress(ap, self.sumExpWithTwoGauss(X, *p_ap))
        return p_ap, r_value
    ...
    def PlotCharge(self):
        ...
        p_ch_sum_two, rv_ch_sum_two = self.ChargePictureSumExpWithTwoGauss() 
        ...
        if rv_ch_two >= rv_ch_sum_two and rv_ch_two >= rv_ch_sum:
            self.PlotTwoGauss(X, p_ch_two)
        ...
        if rv_ch_two <= rv_ch_sum_two and rv_ch_sum <= rv_ch_sum_two:
            self.PlotTwoGaussAndExp(X, p_ch_sum_two)
            LocalMax, IndexMax, LocalMin, IndexMin = fMM.findMaxMin(self.sumExpWithTwoGauss(X, *p_ch_sum_two))
            plt.plot(X, self.sumExpWithTwoGauss(X, *p_ch_sum_two), label=(f'LocalMax = {LocalMax}, '
                    f'MaxCoord = {IndexMax}, '
                    f'LocalMin = {LocalMin}, '
                    f'MinCoord = {IndexMin}'),
                    color='g', linewidth=2)
        ...
        if rv_ch_two <= rv_ch_sum and rv_ch_sum >= rv_ch_sum_two:
            self.PlotGaussAndExp(X, p_ch_sum)
            ...
            plt.plot(X, self.sumExpWithGauss(X, *p_ch_sum), label=(f'LocalMax = {LocalMax}, '
                    ...
                    color='g', linewidth=2)
            plt.legend(fontsize=10)
            plt.title(self.chargename + " Гаусс и экспонента")  
        plt.show()


    def PlotAmpl(self):
        p_ap_two, rv_ap_two = self.AmplePictureTwoGauss() 
        p_ap_sum_two, rv_ap_sum_two = self.AmplePictureSumExpWithTwoGauss() 
        p_ap_sum, rv_ap_sum = self.ChargePictureSumExpWithGauss() 
        X = np.arange(0, len(self.getterdata()[1]), 1)
        fig = plt.figure(figsize=(7, 7))
        plt.xlabel(r'$Заряд,~~пК$', fontsize=14)
        plt.ylabel(r'$Счет,~~шт.$', fontsize=12)
        ...
        if rv_ap_two >= rv_ap_sum_two and rv_ap_two >= rv_ap_sum:
            self.PlotTwoGauss(X, p_ap_two)
        ...
        if rv_ap_two <= rv_ap_sum_two and rv_ap_sum <= rv_ap_sum_two:
            self.PlotTwoGaussAndExp(X, p_ap_sum_two)
            LocalMax, IndexMax, LocalMin, IndexMin = fMM.findMaxMin(self.sumExpWithTwoGauss(X, *p_ap_sum_two))
            plt.plot(X, self.sumExpWithTwoGauss(X, *p_ap_sum_two), label=(f'LocalMax = {LocalMax}, '
                    f'MaxCoord = {IndexMax}, '
                    f'LocalMin = {LocalMin}, '
                    f'MinCoord = {IndexMin}'),
                    color='g', linewidth=2)
        ...
        if rv_ap_two <= rv_ap_sum and rv_ap_sum >= rv_ap_sum_two:
            self.PlotGaussAndExp(X, p_ap_sum)
            ...
            plt.plot(X, self.sumExpWithGauss(X, *p_ap_sum), label=(f'LocalMax = {LocalMax}, '
                    ...
                    color='g', linewidth=2)
            plt.legend(fontsize=10)
            plt.title(self.histname + " Гаусс и экспонента")  
        plt.show()
